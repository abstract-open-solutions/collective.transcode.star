...
class TranscodeTool(BTreeContainer):
    ...
    def delete(self, obj, fieldNames=[], force=False):
        ...
        try:
            transcodeServer = xmlrpclib.ServerProxy(address)
            transcodeServer.getAvailableProfiles()
        except Exception, e:
            log.error(u"Could not connect to transcode daemon %s: %s" % (address, e))
            return
    ...
    def getNextDaemon(self):
        ...
        """
        settings = get_settings()
        daemons = settings.daemon_address
        if len(daemons) == 0:
    ...
    def getProfiles(self):
        ...
        """
        settings = get_settings()
        return settings.transcode_profiles
    ...
    def is_pending(self, UID, fieldName, profile, md5sum, timeout=14400):
        ...
        if (datetime.now() - entry['start']).seconds < timeout:
            msg = u'transcode entry timed out: (%s, %s, %s, %s)' % (UID,
                                                                    fieldName,
                                                                    profile,
                                                                    md5sum)
            log.warn(msg)
    ...
    def is_transcoded(self, UID, fieldName, profile, md5sum, timeout=14400):
        ...
        if entry['status'] != 'ok' or \
                entry['md5'] != md5sum or not entry['path']:
            return False
    ...
    def callback(self, result):
        ...
        if entry['status'] != 'pending':
            msg = 'entry status was not pending, \
            that should not happen: %s' % entry
            log.warn(msg)
    ...
    def validate(self, result):
        ...
        try:
            ...
            msg = "No entry for fieldName %s\
             and profile %s" % (fieldName,
                                result['profile'])
            log.error(msg)
