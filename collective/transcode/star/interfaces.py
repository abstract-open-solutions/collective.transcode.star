...
class ITranscodeSettings(Interface):
    ...
    transcode_profiles = schema.Tuple(
        title=u'Transcode profiles',
    ...
    portal_types = schema.Tuple(
        title=u'Portal types to transcode',
        value_type=schema.TextLine(title=u''),
        default=(u'File',),
    )

    mime_types = schema.Tuple(
        title=u'Supported mime types',
        value_type=schema.TextLine(title=u''),
        default=(
            'application/ogg',
            'video/ogg',
    ...
    secret = schema.TextLine(
        title=u'Shared secret with transcode daemon(s)',
        default=u'1771d99931264d538e75eeb19da7d6a0',
    )

    subtitles = schema.Bool(
        title=u'Show subtitles',
        default=True,
    )

    showDownload = schema.Bool(
        title=u'Show download button to embedded clips',
        default=True,
    )
